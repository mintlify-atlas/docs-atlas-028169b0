...
<Note>
  The default local setup uses **SQLite** for the database — no PostgreSQL installation is required to get started. For a full production-equivalent environment, see the [Local Development Guide](/local-development).
</Note>
...
| **PostgreSQL** | 14+ | Optional locally; SQLite used by default |
...
<Steps>
  ...
  <Step title="Configure Environment Variables">
    ...
    Open `.env` and update these two required values before proceeding:

    ```bash .env
    # Required — set a secure random string
    APP_SECRET_KEY=change-me-locally

    # Required — points to your local PostgreSQL instance
    # Leave as-is to use the default local PostgreSQL setup:
    DATABASE_URL=postgresql://postgres:postgres@localhost:5432/workforce_intelligence_utf8

    # Bootstrap admin credentials — used to create the first admin user
    ...
    <Tip>
      To test locally with SQLite instead of PostgreSQL, comment out the `DATABASE_URL` line in `.env`. The backend will fall back to a local `workforce_intelligence.db` SQLite file automatically.
    </Tip>
  </Step>
  ...
  <Step title="Set Up the Frontend">
    ...
    npm workspaces automatically installs dependencies for `apps/web`, `apps/mobile`, and all `packages/` in a single command.
  </Step>
